...
import { PeriodStatusICellRenderer } from '@/components/business/PeriodStatusBadge';
import type { SingleRowDto } from '@/types/dto/SingleRowDto';
...
const ActionMenu: React.FC<ActionMenuParams> = (({ data, context }) => {
   ...
   const goToEditWorkorder = (e: React.MouseEvent) => {
      ...
      if (data?.work_order_number) {
         navigate(`/scheduler/${context.asset}/dashboard/${data.work_order_number}`);
      }
   }
...
export function useTableColDefs(): ColDef<SingleRowDto>[] {
  return useMemo((() => {
    const base: ColDef[] = [
      {
